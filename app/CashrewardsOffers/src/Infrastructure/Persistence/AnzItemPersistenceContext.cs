...
namespace CashrewardsOffers.Infrastructure.Persistence
{
    public class AnzItemPersistenceContext : IAnzItemPersistenceContext
    {
        ...
        public async Task<AnzItem> Get(string itemId) =>
            MapToAnzItem(await _documentPersistenceContext.FindFirst(Builders<AnzItemDocument>.Filter.Eq("ItemId", itemId)));

        public async Task Insert(AnzItem anzItem) =>
            await _documentPersistenceContext.Insert(anzItem.Adapt<AnzItemDocument>());

        public async Task Update(AnzItem anzItem) =>
        ...
        public async Task<List<AnzItem>> GetPage(int ? pageSize = null, int pageNumber = 1, long? updatedAfter = null)
        {
            ...
            var items = await _documentPersistenceContext.Find(GetFilter(updatedAfter), options);

            return MapToAnzItems(items);
        }

        public async Task<long> GetCount(long? updatedAfter = null) => await _documentPersistenceContext.Count(GetFilter(updatedAfter));
        ...
        private static FilterDefinition<AnzItemDocument> GetFilter(long? updatedAfter) =>
